...
namespace MakeOBB
{
    ...
    public class Matrix3x3
    {
        ...
        public Matrix3x3 SubtractScalarIdentity(double scalar)
        {
    ...
    /// 3x3행렬 클래스
    /// </summary>
    public class Matrix3x3Complex
    {
        public Complex[,] Mat { get; private set; } = new Complex[3, 3];

        //행렬 생성
        public Matrix3x3Complex(Matrix3x3 m33)
        {
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    Mat[i, j] = new Complex(m33.Mat[i, j], 0);
                }
            }
        }

        public Matrix3x3Complex(Complex[,] data)
        {
            if (data.GetLength(0) != 3 || data.GetLength(1) != 3)
                throw new ArgumentException("Matrix must be 3x3.");
            Array.Copy(data, Mat, 9);//원본 : data
        ...
        //스칼라 곱셈, 단위 행렬 뺄셈 (A-lamda*I)
        public Matrix3x3Complex SubtractScalarIdentity(Complex scalar)
        {
            Complex[,] resultData = new Complex[3, 3]; // 결과 행렬은 double로 유지하되, 내부적으로 Complex 계산
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
            ...
            return new Matrix3x3Complex(resultData);
        }

        //행렬 복사
        public Matrix3x3Complex Clone()
        {
            return new Matrix3x3Complex(Mat);
        }
    }
    ...
    public class EigenVectorDecomposition
    {
        ...
        public Complex[] GetEigenValue(Matrix3x3 m)
        {
            ...
            //해 정렬 - 내림차순
            double[] eigenValuesReal = new double[eigenvalues.Length];
            for (int i = 0; i < eigenvalues.Length; i++)
            {
                eigenValuesReal[i] = eigenvalues[i].Real;
            }
            Array.Reverse(eigenValuesReal);
            return eigenvalues;
        }
        /// <summary>
        /// 고유벡터 구하기
        ...
        /// <returns></returns>
        public double[,] GetEigenVector(Complex[] eigenvalues, Matrix3x3 mat, double epcilon = 1e-12)
        {
            ...
            for (int i = 0; i < eigenvalues.Length; i++)
            {
                //고유값은 복소수 형태로 반환될 수 있지만, 공분산 행렬은 대칭이므로 실수 고유값을 가집니다.
                ...
                Matrix3x3Complex MC = new Matrix3x3Complex(mat);
                Matrix3x3Complex M = MC.SubtractScalarIdentity(lambda);
                //Matrix3x3Complex M = mat.SubtractScalarIdentity(lambda);

                //가우스 죠르단 소거법 사용해서 연립방정식의 해 구하기 => 이 해가 고유벡터
                // 목표: 행렬을 행 사다리꼴(Row Echelon Form)로 만듦

                //Matrix3x3 gauseMat = RowEchelonFormMatrix(M);
                Matrix3x3Complex gauseMat = RowEchelonFormMatrix(MC);
                ...
                if (rank >= 2)
                {
                    if (Math.Abs(gauseMat.Mat[1, 1].Magnitude) > epcilon)
                    {
                        //v[1] = -new Complex(gauseMat.Mat[1, 2], 0.0) * v[2] / new Complex(gauseMat.Mat[1, 1], 0.0);
                        v[1] = -gauseMat.Mat[1, 2] * v[2] / gauseMat.Mat[1, 1];
                    }
        ...
        /// <returns></returns>
        int RankMatrix(Matrix3x3Complex gauseMat, double epcilon = 1e-9)
        {
            int rank = 0;
            //랭크가 3일경우
            if (Math.Abs(gauseMat.Mat[2, 2].Magnitude) > epcilon)
            {
                rank = 3;
            }
            else
            {
                //랭크 1
                if (Math.Abs(gauseMat.Mat[1, 2].Magnitude) < epcilon && Math.Abs(gauseMat.Mat[1, 1].Magnitude) < epcilon)
                    rank = 1;
                else//랭크 2
                    rank = 2;
            }
        ...
        /// <param name="m"></param>
        /// <returns></returns>
        Matrix3x3Complex RowEchelonFormMatrix(Matrix3x3Complex m, double epcilon = 1e-9)
        {
            Matrix3x3Complex curMat = m.Clone();
            ...
            for (int k = 0; k < 2; k++)
            {
                ...
                {
                    if (Math.Abs(m.Mat[row, k].Magnitude) > Math.Abs(m.Mat[pivotRow, k].Magnitude))
                    {
                        pivotRow = row;
                    }
                }
                ...
                if (Math.Abs(curMat.Mat[k, k].Magnitude) < epcilon)
                {
                    curMat.Mat[k, k] = 0;
                    continue;
                }
                ...
                for (int row = k + 1; row < 3; row++)
                {
                    Complex factor = curMat.Mat[row, k] / curMat.Mat[k, k];
        ...
        #region 미사용 로직, 고유벡터 구할 때, 실수형만 적용한 함수들 ,추후 사용여지 있음
        /// <summary>
        /// Rank 구하기
        /// </summary>
        /// <param name="gauseMat"></param>
        /// <returns></returns>
        int RankMatrix(Matrix3x3 gauseMat, double epcilon = 1e-9)
        {
            int rank = 0;
            //랭크가 3일경우
            if (Math.Abs(gauseMat.Mat[2, 2]) > epcilon)
            {
                rank = 3;
            }
            else
            {
                //랭크 1
                if (Math.Abs(gauseMat.Mat[1, 2]) < epcilon && Math.Abs(gauseMat.Mat[1, 1]) < epcilon)
                    rank = 1;
                else//랭크 2
                    rank = 2;
            }
