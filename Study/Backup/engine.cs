...
namespace NWH.DWP2.ShipController
{
    ...
    public class Engine
    {
        ...
        private bool  _wasOn;
        private float _lastThrottleDirection; 
        ...
        public virtual void Update()
        {
            ...
            // Store last throttle direction, �ù����϶� ������
            if (Mathf.Abs(throttleInput) > 0.01f)
            {
                _lastThrottleDirection = Mathf.Sign(throttleInput);
            }
            ...
            //SimulationOnOff();//�ùķ��̼� ���¿� ���� ���� �������
            // Thrust calculation is now based on RPM, not instant throttle input
            if (_engineState == State.On || _engineState == State.Stopping)
            {
                ...
                Thrust = 0;
                // && RPM > minRPM + 1f
                if (applyForce && maxRPM != 0 && maxSpeed != 0 && RPM > minRPM + 1f)
                {
                    Thrust = _lastThrottleDirection * (_rpm / maxRPM) *
                             thrustCurve.Evaluate(Mathf.Abs(_sc.Speed) / maxSpeed) * maxThrust;
                    Thrust = _lastThrottleDirection >= 0 ? Thrust : Thrust * reverseThrustCoefficient;
            ...
            {
                Thrust = 0; // Ensure thrust is zero when engine is fully off
            }
        ...
        public void SimulationOnOff()
        {
            if (_engineState == State.On)
            {
                _lastThrottleDirection = 1;
            }
            if (_engineState == State.Off)
            {
                _lastThrottleDirection = 0;
            }
        }
    }
}
