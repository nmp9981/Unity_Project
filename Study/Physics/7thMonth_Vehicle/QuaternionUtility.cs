...
{
    //3*3행렬을 쿼터니언으로 변환
    public static CustomQuaternion Mat3ToQuaternion(Mat3 m)
    {
        float w = MathUtility.Root(1 + m.m00 + m.m11 + m.m22)*0.5f;
        float x = m.m21 - m.m12/(4*w);
        float y = m.m02 - m.m20 / (4 * w);
        float z = m.m10 - m.m01 / (4 * w);

        CustomQuaternion newQuaternion = new CustomQuaternion
        {
            scala = w,
            vec = new Vec3 (x, y, z)
        };
