public class BoxCollider3D : CustomCollider3D
{
    public Vec3 halfExtent;
    public override CustomColliderType Type => CustomColliderType.Box;
    ...
    public override bool RayCast(Ray3D ray, float maxT, out RaycastHit3D hit)
    {
        ...
        Ray3D localRay;
        localRay.origin = MatrixUtility.MulPoint(ray.origin, transform3D.WorldToLocal);
        localRay.dir = MatrixUtility.MulVector(ray.dir, transform3D.WorldToLocal);
        localRay.dir = localRay.dir.Normalized;//정규화
        ...
        Vec3 localHitPos = localRay.origin + hit.t * localRay.dir;//월드 좌표에서의 충돌 지점
        hit.position = MatrixUtility.MulPoint(localHitPos, transform3D.LocalToWorld);
        ...
        out SweepHit hit)
    {
        ...
        return SphereSweep.SweepSphereOBB(
            center,
            radius,
            dir,
            maxT,
